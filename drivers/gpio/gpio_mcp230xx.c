...
/**
 * @file Driver for MPC230xx I2C/SPI-based GPIO driver.
 */
...
static int mcp230xx_read_port_regs(const struct device *dev, uint8_t reg, uint16_t *buf)
{
	...
	ret = i2c_burst_read_dt(&config->bus.i2c, reg, (uint8_t *)&port_data,
			     nread);
	if (ret < 0) {
...
static int mcp230xx_write_port_regs(const struct device *dev, uint8_t reg, uint16_t value)
{
	...
	ret = i2c_burst_write_dt(&config->bus.i2c, reg, (uint8_t *)&port_data,
			      nwrite);
	if (ret < 0) {
...
#define GPIO_MCP230XX_DEVICE(n)                                                      \
	...
	DEVICE_DT_INST_DEFINE(n, gpio_mcp23xxx_init, NULL,                      \
			 &mcp230xx_##inst##_drvdata, &mcp230xx_##inst##_config,  \
			 POST_KERNEL, CONFIG_GPIO_MCP230XX_INIT_PRIORITY, &gpio_mcp23xxx_api_table);
